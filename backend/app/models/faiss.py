...
def get_embedding_from_image_bytes_faiss(image) -> np.ndarray:
...
def predict_price_faiss(image) -> float:
    emb = get_embedding_from_image_bytes_faiss(image).reshape(1, -1)
...
def get_top3_similar_item_ids_faiss(image) -> list[int]:
    emb = get_embedding_from_image_bytes_faiss(image).reshape(1, -1)
